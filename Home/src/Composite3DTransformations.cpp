...
void compositeTransform(Point3D* points, int n, float tx, float ty, float tz, float angleX, float angleY, float angleZ, float sx, float sy, float sz) {
    translate(points, n, tx, ty, tz); // Apply translation
    rotate(points, n, angleX, angleY, angleZ); // Apply rotation
    scale(points, n, sx, sy, sz); // Apply scaling
}
...
int main() {
    ...
    compositeTransform(points, n, 50, 50, 0, 45, 0, 0, 1.5, 1.5, 1.5); // Translate, Rotate, Scale
    setcolor(RED);
    draw3DShape(points, 4); // Draw base after transformations
    line(points[0].x + 300, 300 - points[0].y, points[4].x + 300, 300 - points[4].y);
    line(points[1].x + 300, 300 - points[1].y, points[4].x + 300, 300 - points[4].y);
    line(points[2].x + 300, 300 - points[2].y, points[4].x + 300, 300 - points[4].y);
    line(points[3].x + 300, 300 - points[3].y, points[4].x + 300, 300 - points[4].y);
